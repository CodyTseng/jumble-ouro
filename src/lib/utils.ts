...
export function getClientDescription(): string {
  if (typeof window === 'undefined' || !window.navigator) return 'Jumble'
  ...
  const details = [os, browser].filter(Boolean).join(', ')
  return details ? `Jumble (${details})` : 'Jumble'
}
